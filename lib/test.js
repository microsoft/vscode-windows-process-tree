...
describe('getRawProcessList', () => {
    ...
    it('should handle multiple calls gracefully', (done) => {
        ...
        native.getProcessList(callback, index_1.ProcessDataFlag.None);
        native.getProcessList(callback, index_1.ProcessDataFlag.None);
    });
    it('should return memory information only when the flag is set', (done) => {
        ...
        native.getProcessList((list) => {
            ...
            }, index_1.ProcessDataFlag.Memory);
        }, index_1.ProcessDataFlag.None);
    });
    it('should return command line information only when the flag is set', (done) => {
        ...
        native.getProcessList((list) => {
            ...
            }, index_1.ProcessDataFlag.CommandLine);
        }, index_1.ProcessDataFlag.None);
    });
});
describe('getProcessList', () => {
    ...
    it('should return a list containing this process', (done) => {
        index_1.getProcessList(process.pid, (list) => {
...
describe('getProcessCpuUsage', () => {
    it('should get process cpu usage', (done) => {
        index_1.getProcessCpuUsage([{ pid: process.pid, ppid: process.ppid, name: 'node.exe' }], (annotatedList) => {
    ...
    it('should handle multiple calls gracefully', function (done) {
        this.timeout(3000);
        let counter = 0;
        const callback = (list) => {
            assert.notEqual(list.find(p => p.pid === process.pid), undefined);
        ...
        index_1.getProcessCpuUsage([{ pid: process.pid, ppid: process.ppid, name: 'node.exe' }], callback);
        index_1.getProcessCpuUsage([{ pid: process.pid, ppid: process.ppid, name: 'node.exe' }], callback);
    });
});
describe('getProcessTree', () => {
    ...
    it('should return a tree containing this process', done => {
        index_1.getProcessTree(process.pid, (tree) => {
...
describe('buildProcessTree', () => {
    it('should enforce a maximum search depth', () => {
        const tree = index_1.buildProcessTree(0, [
            { pid: 0, ppid: 0, name: '0' }
        ], 3);
...
describe('filterProcessList', () => {
    it('should enforce a maximum search depth', () => {
        const list = index_1.filterProcessList(0, [
            { pid: 0, ppid: 0, name: '0' }
        ], 3);
